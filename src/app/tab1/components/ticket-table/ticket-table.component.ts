...
export class TicketTableComponent implements OnInit, OnDestroy {
  ...
  public initiateForm(): FormGroup {
    return this.fb.group({
      ...
      ticketPhone: this.fb.control('065423554'),
  ...
  public async printInvoiceModal(): Promise<void> {
    const alert: HTMLIonAlertElement = await this.alertCtrl.create({
      ...
      buttons: [
        ...
        {
          text: 'Printaj bez troškova',
          handler: () => {
            this.printInvoiceTax(false);
          },
        },
        {
          text: 'Printaj sa troškovima',
          handler: () => {
            this.printInvoiceTax(true);
          },
        },
      ],
    });
